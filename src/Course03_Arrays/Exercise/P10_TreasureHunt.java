    ...
    public static void main(String[] args) {
        ...
        while (!command.equals("Yohoho!")) {
            ...
            switch (commandParts[0]) {
                ...
                case "Steal":
                    int numberOfStolenItems = Integer.parseInt(commandParts[1]);

                    if (numberOfStolenItems >= 0 && numberOfStolenItems < treasureChest.length) {
                        for (int i = 0; i < numberOfStolenItems; i++) {
                            System.out.print(treasureChest[treasureChest.length - numberOfStolenItems + i]);
                            if (i != numberOfStolenItems - 1) {
                                System.out.print(", ");
                            }
                        }
                        String[] tempChest = new String[treasureChest.length - numberOfStolenItems];
                        for (int i = 0; i < tempChest.length; i++) {
                            tempChest[i] = treasureChest[i];
                        }
                        treasureChest = tempChest;

                    } else if (numberOfStolenItems >= 0) {
